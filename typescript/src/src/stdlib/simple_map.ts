...
export function add_ (
  ...
  Vector.push_back_(map.data, new Element({ key: key, value: value }, new SimpleStructTag(Element, [$p[0], $p[1]])), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
  placement = Option.extract_(maybe_placement, $c, [AtomicTypeTag.U64]);
  end = (Vector.length_(map.data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])])).sub(u64("1"));
  while (($.copy(placement)).lt($.copy(end))) {
    {
      Vector.swap_(map.data, $.copy(placement), $.copy(end), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
...
export function borrow_ (
  ...
  return Vector.borrow_(map.data, $.copy(idx), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]).value;
}
...
export function borrow_mut_ (
  ...
  idx = Option.extract_(maybe_idx, $c, [AtomicTypeTag.U64]);
  return Vector.borrow_mut_(map.data, $.copy(idx), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]).value;
}
...
export function create_ (
  ...
  return new SimpleMap({ data: Vector.empty_($c, [new SimpleStructTag(Element, [$p[0], $p[1]])]) }, new SimpleStructTag(SimpleMap, [$p[0], $p[1]]));
}
...
export function destroy_empty_ (
  ...
  let { data: data } = map;
  Vector.destroy_empty_(data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
  return;
}
...
export function find_ (
  ...
  length = Vector.length_(map.data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
  if (($.copy(length)).eq((u64("0")))) {
    return [Option.none_($c, [AtomicTypeTag.U64]), Option.some_(u64("0"), $c, [AtomicTypeTag.U64])];
  }
  else{
  }
  left = u64("0");
  ...
  while (($.copy(left)).neq($.copy(right))) {
    {
      ...
      potential_key = Vector.borrow_(map.data, $.copy(mid), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]).key;
    ...
    temp$2 = $.dyn_eq($p[0], key, Vector.borrow_(map.data, $.copy(left), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]).key);
  }
  else{
    temp$2 = false;
  }
  if (temp$2) {
    [temp$3, temp$4] = [Option.some_($.copy(left), $c, [AtomicTypeTag.U64]), Option.none_($c, [AtomicTypeTag.U64])];
  }
  else{
    [temp$3, temp$4] = [Option.none_($c, [AtomicTypeTag.U64]), Option.some_($.copy(left), $c, [AtomicTypeTag.U64])];
  }
...
export function length_ (
  ...
  return Vector.length_(map.data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
}
...
export function remove_ (
  ...
  while (($.copy(placement)).lt($.copy(end))) {
    {
      Vector.swap_(map.data, $.copy(placement), ($.copy(placement)).add(u64("1")), $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
      placement = ($.copy(placement)).add(u64("1"));
    }

  }let { key: key__3, value: value } = Vector.pop_back_(map.data, $c, [new SimpleStructTag(Element, [$p[0], $p[1]])]);
  return [key__3, value];
}
