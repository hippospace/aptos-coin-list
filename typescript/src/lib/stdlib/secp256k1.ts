...
export class ECDSARawPublicKey 
{
  ...
  static structName: string = "ECDSARawPublicKey";
...
export class ECDSASignature 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "ECDSASignature";
  static typeParameters: TypeParamDeclType[] = [
...
export function ecdsa_raw_public_key_from_64_bytes_ (
  ...
  if (!(Vector.length_(bytes, $c, [AtomicTypeTag.U8])).eq(($.copy(RAW_PUBLIC_KEY_NUM_BYTES)))) {
    throw $.abortCode(Error.invalid_argument_($.copy(E_DESERIALIZE), $c));
  }
  return new ECDSARawPublicKey({ bytes: $.copy(bytes) }, new SimpleStructTag(ECDSARawPublicKey));
}
...
export function ecdsa_recover_ (
  ...
  if (success) {
    temp$1 = Option.some_(ecdsa_raw_public_key_from_64_bytes_($.copy(pk), $c), $c, [new SimpleStructTag(ECDSARawPublicKey)]);
  }
  else{
...
export function ecdsa_signature_from_bytes_ (
  ...
  return new ECDSASignature({ bytes: $.copy(bytes) }, new SimpleStructTag(ECDSASignature));
}
