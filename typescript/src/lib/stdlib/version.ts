...
export class SetVersionCapability 
{
  ...
  static structName: string = "SetVersionCapability";
  ...
  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, SetVersionCapability, typeParams);
    return result as unknown as SetVersionCapability;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, SetVersionCapability, typeParams);
...
export function initialize_ (
  ...
  $c.move_to(new SimpleStructTag(Version), aptos_framework, new Version({ major: $.copy(initial_version) }, new SimpleStructTag(Version)));
  $c.move_to(new SimpleStructTag(SetVersionCapability), aptos_framework, new SetVersionCapability({  }, new SimpleStructTag(SetVersionCapability)));
  return;
}
...
export function initialize_for_test_ (
  ...
  $c.move_to(new SimpleStructTag(SetVersionCapability), core_resources, new SetVersionCapability({  }, new SimpleStructTag(SetVersionCapability)));
  return;
}
...
export function set_version_ (
  ...
  if (!$c.exists(new SimpleStructTag(SetVersionCapability), Signer.address_of_(account, $c))) {
  ...
  if (!($.copy(old_major)).lt($.copy(major))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINVALID_MAJOR_VERSION_NUMBER), $c));
  }
