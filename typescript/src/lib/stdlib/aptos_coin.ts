...
export function claim_mint_capability_ (
  ...
  Vector.swap_remove_(delegations, $.copy(idx), $c, [new SimpleStructTag(DelegatedMintCapability)]);
...
export function configure_accounts_for_test_ (
  ...
  Coin.deposit_(Signer.address_of_(core_resources, $c), coins, $c, [new SimpleStructTag(AptosCoin)]);
  ...
  $c.move_to(new SimpleStructTag(Delegations), core_resources, new Delegations({ inner: Vector.empty_($c, [new SimpleStructTag(DelegatedMintCapability)]) }, new SimpleStructTag(Delegations)));
  return;
}
...
export function delegate_mint_capability_ (
  ...
  delegations = $c.borrow_global_mut<Delegations>(new SimpleStructTag(Delegations), new HexString("0xa550c18")).inner;
  i = u64("0");
  while (($.copy(i)).lt(Vector.length_(delegations, $c, [new SimpleStructTag(DelegatedMintCapability)]))) {
    {
      [temp$1, temp$2] = [delegations, $.copy(i)];
      element = Vector.borrow_(temp$1, temp$2, $c, [new SimpleStructTag(DelegatedMintCapability)]);
      if (!(($.copy(element.to)).hex() !== ($.copy(to)).hex())) {
        throw $.abortCode(Error.invalid_argument_($.copy(EALREADY_DELEGATED), $c));
      }
      i = ($.copy(i)).add(u64("1"));
    }

  }Vector.push_back_(delegations, new DelegatedMintCapability({ to: $.copy(to) }, new SimpleStructTag(DelegatedMintCapability)), $c, [new SimpleStructTag(DelegatedMintCapability)]);
  return;
}
...
export function find_delegation_ (
  ...
  i = u64("0");
  len = Vector.length_(delegations, $c, [new SimpleStructTag(DelegatedMintCapability)]);
  index = Option.none_($c, [AtomicTypeTag.U64]);
  while (($.copy(i)).lt($.copy(len))) {
    {
      element = Vector.borrow_(delegations, $.copy(i), $c, [new SimpleStructTag(DelegatedMintCapability)]);
...
export function initialize_ (
  ...
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  [burn_cap, freeze_cap, mint_cap] = Coin.initialize_with_parallelizable_supply_(aptos_framework, String.utf8_([u8("65"), u8("112"), u8("116"), u8("111"), u8("115"), u8("32"), u8("67"), u8("111"), u8("105"), u8("110")], $c), String.utf8_([u8("65"), u8("80"), u8("84")], $c), u8("8"), true, $c, [new SimpleStructTag(AptosCoin)]);
