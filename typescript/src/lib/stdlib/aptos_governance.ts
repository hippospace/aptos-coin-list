...
export class GovernanceEvents 
{
  ...
  constructor(proto: any, public typeTag: TypeTag) {
    ...
    this.update_config_events = proto['update_config_events'] as Event.EventHandle;
...
export function add_approved_script_hash_ (
  ...
  if (!($.copy(proposal_state)).eq(($.copy(PROPOSAL_STATE_SUCCEEDED)))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EPROPOSAL_NOT_RESOLVABLE_YET), $c));
  }
  ...
  Simple_map.add_(approved_hashes.hashes, $.copy(proposal_id), $.copy(execution_hash), $c, [AtomicTypeTag.U64, new VectorTag(AtomicTypeTag.U8)]);
  return;
}
...
export function create_proposal_ (
  ...
  if (!($.copy(stake_balance)).ge($.copy(governance_config.required_proposer_stake))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINSUFFICIENT_PROPOSER_STAKE), $c));
  }
  ...
  if (!(Stake.get_lockup_secs_($.copy(stake_pool), $c)).ge($.copy(proposal_expiration))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EINSUFFICIENT_STAKE_LOCKUP), $c));
  }
  ...
  if (Option.is_some_(total_voting_token_supply, $c, [AtomicTypeTag.U128])) {
    total_supply = $.copy(Option.borrow_(total_voting_token_supply, $c, [AtomicTypeTag.U128]));
    early_resolution_vote_threshold = Option.some_((($.copy(total_supply)).div(u128("2"))).add(u128("1")), $c, [AtomicTypeTag.U128]);
  }
  else{
  }
  ...
  Event.emit_event_(temp$6, new CreateProposalEvent({ proposer: temp$2, stake_pool: temp$3, proposal_id: temp$1, execution_hash: temp$4, proposal_metadata: temp$5 }, new SimpleStructTag(CreateProposalEvent)), $c, [new SimpleStructTag(CreateProposalEvent)]);
  return;
}
...
export function create_proposal_metadata_ (
  ...
  if (!(String.length_(temp$1, $c)).le(u64("256"))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EMETADATA_LOCATION_TOO_LONG), $c));
  }
  temp$2 = String.utf8_($.copy(metadata_hash), $c);
  if (!(String.length_(temp$2, $c)).le(u64("256"))) {
    throw $.abortCode(Error.invalid_argument_($.copy(EMETADATA_HASH_TOO_LONG), $c));
  }
  metadata = Simple_map.create_($c, [new StructTag(new HexString("0x1"), "string", "String", []), new VectorTag(AtomicTypeTag.U8)]);
  Simple_map.add_(metadata, String.utf8_($.copy(METADATA_LOCATION_KEY), $c), $.copy(metadata_location), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new VectorTag(AtomicTypeTag.U8)]);
  Simple_map.add_(metadata, String.utf8_($.copy(METADATA_HASH_KEY), $c), $.copy(metadata_hash), $c, [new StructTag(new HexString("0x1"), "string", "String", []), new VectorTag(AtomicTypeTag.U8)]);
...
export function initialize_ (
  ...
  $c.move_to(new SimpleStructTag(GovernanceEvents), aptos_framework, new GovernanceEvents({ create_proposal_events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(CreateProposalEvent)]), update_config_events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(UpdateConfigEvent)]), vote_events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(VoteEvent)]) }, new SimpleStructTag(GovernanceEvents)));
  $c.move_to(new SimpleStructTag(VotingRecords), aptos_framework, new VotingRecords({ votes: Table.new___($c, [new SimpleStructTag(RecordKey), AtomicTypeTag.Bool]) }, new SimpleStructTag(VotingRecords)));
...
export function remove_approved_hash_ (
  ...
  if (!Voting.is_resolved_(new HexString("0x1"), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "governance_proposal", "GovernanceProposal", [])])) {
    throw $.abortCode(Error.invalid_argument_($.copy(EPROPOSAL_NOT_RESOLVED_YET), $c));
  }
  ...
  if (Simple_map.contains_key_(temp$1, temp$2, $c, [AtomicTypeTag.U64, new VectorTag(AtomicTypeTag.U8)])) {
    Simple_map.remove_(approved_hashes, proposal_id, $c, [AtomicTypeTag.U64, new VectorTag(AtomicTypeTag.U8)]);
  }
  else{
  }
  return;
...
export function store_signer_cap_ (
  ...
  if (!$c.exists(new SimpleStructTag(GovernanceResponsbility), new HexString("0x1"))) {
    $c.move_to(new SimpleStructTag(GovernanceResponsbility), aptos_framework, new GovernanceResponsbility({ signer_caps: Simple_map.create_($c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])]) }, new SimpleStructTag(GovernanceResponsbility)));
  }
  else{
  }
  ...
  Simple_map.add_(signer_caps, $.copy(signer_address), signer_cap, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])]);
  return;
}
...
export function update_governance_config_ (
  ...
  events = $c.borrow_global_mut<GovernanceEvents>(new SimpleStructTag(GovernanceEvents), new HexString("0x1"));
  Event.emit_event_(events.update_config_events, new UpdateConfigEvent({ min_voting_threshold: $.copy(min_voting_threshold), required_proposer_stake: $.copy(required_proposer_stake), voting_duration_secs: $.copy(voting_duration_secs) }, new SimpleStructTag(UpdateConfigEvent)), $c, [new SimpleStructTag(UpdateConfigEvent)]);
  return;
}
...
export function vote_ (
  ...
  if (!!Table.contains_(voting_records.votes, $.copy(record_key), $c, [new SimpleStructTag(RecordKey), AtomicTypeTag.Bool])) {
    throw $.abortCode(Error.invalid_argument_($.copy(EALREADY_VOTED), $c));
  }
  Table.add_(voting_records.votes, $.copy(record_key), true, $c, [new SimpleStructTag(RecordKey), AtomicTypeTag.Bool]);
