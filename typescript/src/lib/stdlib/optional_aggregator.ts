...
export class Integer 
{
  ...
  static structName: string = "Integer";
...
export class OptionalAggregator 
{
  ...
  static moduleName = moduleName;
  __app: $.AppType | null = null;
  static structName: string = "OptionalAggregator";
  static typeParameters: TypeParamDeclType[] = [

  ];
  static fields: FieldDeclType[] = [
  { name: "aggregator", typeTag: new StructTag(new HexString("0x1"), "option", "Option", [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]) },
  { name: "integer", typeTag: new StructTag(new HexString("0x1"), "option", "Option", [new StructTag(new HexString("0x1"), "optional_aggregator", "Integer", [])]) }];
...
export function add_ (
  ...
  if (Option.is_some_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])])) {
    aggregator = Option.borrow_mut_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
    Aggregator.add_(aggregator, $.copy(value), $c);
  }
  else{
    integer = Option.borrow_mut_(optional_aggregator.integer, $c, [new SimpleStructTag(Integer)]);
...
export function destroy_integer_ (
  integer: Integer,
  $c: AptosDataCache,
): void {
  integer;
...
export function destroy_optional_aggregator_ (
  ...
  limit = Aggregator.limit_(Option.borrow_(aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]), $c);
  Aggregator.destroy_(Option.destroy_some_(aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]), $c);
...
export function destroy_optional_integer_ (
  ...
  let limit;
  let { aggregator: aggregator, integer: integer } = optional_aggregator;
  limit = limit_(Option.borrow_(integer, $c, [new SimpleStructTag(Integer)]), $c);
  destroy_integer_(Option.destroy_some_(integer, $c, [new SimpleStructTag(Integer)]), $c);
  Option.destroy_none_(aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
  return $.copy(limit);
}
...
export function is_parallelizable_ (
  ...
  return Option.is_some_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
}

export function limit_ (
  integer: Integer,
  $c: AptosDataCache,
): U128 {
...
export function new___ (
  ...
  let temp$1;
  if (parallelizable) {
    temp$1 = new OptionalAggregator({ aggregator: Option.some_(Aggregator_factory.create_aggregator_internal_($.copy(limit), $c), $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]), integer: Option.none_($c, [new SimpleStructTag(Integer)]) }, new SimpleStructTag(OptionalAggregator));
  }
  else{
    temp$1 = new OptionalAggregator({ aggregator: Option.none_($c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]), integer: Option.some_(new_integer_($.copy(limit), $c), $c, [new SimpleStructTag(Integer)]) }, new SimpleStructTag(OptionalAggregator));
  }
  return temp$1;
}

export function new_integer_ (
  limit: U128,
  $c: AptosDataCache,
): Integer {
  return new Integer({ value: u128("0"), limit: $.copy(limit) }, new SimpleStructTag(Integer));
}
...
export function read_ (
  ...
  if (Option.is_some_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])])) {
    aggregator = Option.borrow_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
    temp$1 = Aggregator.read_(aggregator, $c);
  }
  else{
    integer = Option.borrow_(optional_aggregator.integer, $c, [new SimpleStructTag(Integer)]);
    temp$1 = read_integer_(integer, $c);
  }
  return temp$1;
}

export function read_integer_ (
  integer: Integer,
  $c: AptosDataCache,
): U128 {
...
export function switch_to_aggregator_and_zero_out_ (
  ...
  integer = Option.extract_(optional_aggregator.integer, $c, [new SimpleStructTag(Integer)]);
  limit = limit_(integer, $c);
  destroy_integer_(integer, $c);
  aggregator = Aggregator_factory.create_aggregator_internal_($.copy(limit), $c);
  Option.fill_(optional_aggregator.aggregator, aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
  return $.copy(limit);
}
...
export function switch_to_integer_and_zero_out_ (
  ...
  let aggregator, integer, limit;
  aggregator = Option.extract_(optional_aggregator.aggregator, $c, [new StructTag(new HexString("0x1"), "aggregator", "Aggregator", [])]);
  limit = Aggregator.limit_(aggregator, $c);
  Aggregator.destroy_(aggregator, $c);
  integer = new_integer_($.copy(limit), $c);
  Option.fill_(optional_aggregator.integer, integer, $c, [new SimpleStructTag(Integer)]);
  return $.copy(limit);
}
