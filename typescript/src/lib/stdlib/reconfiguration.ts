...
export function current_epoch_ (
  $c: AptosDataCache,
): U64 {
  return $.copy($c.borrow_global<Configuration>(new SimpleStructTag(Configuration), new HexString("0x1")).epoch);
}
...
export function disable_reconfiguration_ (
  ...
  return $c.move_to(new SimpleStructTag(DisableReconfiguration), aptos_framework, new DisableReconfiguration({  }, new SimpleStructTag(DisableReconfiguration)));
}
...
export function enable_reconfiguration_ (
  ...
  $c.move_from<DisableReconfiguration>(new SimpleStructTag(DisableReconfiguration), Signer.address_of_(aptos_framework, $c));
  return;
}
...
export function initialize_ (
  ...
  System_addresses.assert_aptos_framework_(aptos_framework, $c);
  if (!(Account.get_guid_next_creation_num_(Signer.address_of_(aptos_framework, $c), $c)).eq((u64("2")))) {
    throw $.abortCode(Error.invalid_state_($.copy(EINVALID_GUID_FOR_EVENT), $c));
  }
  $c.move_to(new SimpleStructTag(Configuration), aptos_framework, new Configuration({ epoch: u64("0"), last_reconfiguration_time: u64("0"), events: Account.new_event_handle_(aptos_framework, $c, [new SimpleStructTag(NewEpochEvent)]) }, new SimpleStructTag(Configuration)));
  return;
}
