...
export class AggregatorFactory 
{
  ...
  static structName: string = "AggregatorFactory";
  static typeParameters: TypeParamDeclType[] = [

  ];
  static fields: FieldDeclType[] = [
  { name: "phantom_table", typeTag: new StructTag(new HexString("0x1"), "table", "Table", [AtomicTypeTag.Address, AtomicTypeTag.U128]) }];
  ...
  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, AggregatorFactory, typeParams);
    return result as unknown as AggregatorFactory;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, AggregatorFactory, typeParams);
...
export function create_aggregator_internal_ (
  ...
  if (!$c.exists(new SimpleStructTag(AggregatorFactory), new HexString("0x1"))) {
    throw $.abortCode(Error.not_found_($.copy(EAGGREGATOR_FACTORY_NOT_FOUND), $c));
  }
  aggregator_factory = $c.borrow_global_mut<AggregatorFactory>(new SimpleStructTag(AggregatorFactory), new HexString("0x1"));
...
export function initialize_aggregator_factory_ (
  ...
  aggregator_factory = new AggregatorFactory({ phantom_table: Table.new___($c, [AtomicTypeTag.Address, AtomicTypeTag.U128]) }, new SimpleStructTag(AggregatorFactory));
  $c.move_to(new SimpleStructTag(AggregatorFactory), aptos_framework, aggregator_factory);
  return;
}
...
export class App {
  ...
  async loadAggregatorFactory(
    ...
    const val = await AggregatorFactory.load(this.repo, this.client, owner, [] as TypeTag[]);
    if (loadFull) {
