...
export class GenesisEndMarker 
{
  ...
  static structName: string = "GenesisEndMarker";
  ...
  static async load(repo: AptosParserRepo, client: AptosClient, address: HexString, typeParams: TypeTag[]) {
    const result = await repo.loadResource(client, address, GenesisEndMarker, typeParams);
    return result as unknown as GenesisEndMarker;
  }
  static async loadByApp(app: $.AppType, address: HexString, typeParams: TypeTag[]) {
    const result = await app.repo.loadResource(app.client, address, GenesisEndMarker, typeParams);
...
export function set_genesis_end_ (
  ...
  $c.move_to(new SimpleStructTag(GenesisEndMarker), aptos_framework, new GenesisEndMarker({  }, new SimpleStructTag(GenesisEndMarker)));
  return;
}
