...
export function aggregate_pubkeys_ (
  ...
  if (!success) {
    throw $.abortCode(Std.Error.invalid_argument_($.copy(EZERO_PUBKEYS), $c));
  }
...
export function aggregate_signatures_ (
  ...
  if (success) {
    temp$1 = Std.Option.some_(new AggrOrMultiSignature({ bytes: $.copy(bytes) }, new SimpleStructTag(AggrOrMultiSignature)), $c, [new SimpleStructTag(AggrOrMultiSignature)]);
  }
  else{
    temp$1 = Std.Option.none_($c, [new SimpleStructTag(AggrOrMultiSignature)]);
  }
  return temp$1;
}
...
export function public_key_from_bytes_ (
  ...
  if (validate_pubkey_internal_($.copy(bytes), $c)) {
    temp$1 = Std.Option.some_(new PublicKey({ bytes: $.copy(bytes) }, new SimpleStructTag(PublicKey)), $c, [new SimpleStructTag(PublicKey)]);
  }
  else{
    temp$1 = Std.Option.none_($c, [new SimpleStructTag(PublicKey)]);
  }
  return temp$1;
}
...
export function public_key_from_bytes_with_pop_ (
  ...
  if (verify_proof_of_possession_internal_($.copy(pk_bytes), $.copy(pop.bytes), $c)) {
    temp$1 = Std.Option.some_(new PublicKeyWithPoP({ bytes: $.copy(pk_bytes) }, new SimpleStructTag(PublicKeyWithPoP)), $c, [new SimpleStructTag(PublicKeyWithPoP)]);
  }
  else{
    temp$1 = Std.Option.none_($c, [new SimpleStructTag(PublicKeyWithPoP)]);
  }
  return temp$1;
}
