...
export function new_signature_from_bytes_ (
  ...
  if (!(Std.Vector.length_(bytes, $c, [AtomicTypeTag.U8])).eq(($.copy(SIGNATURE_NUM_BYTES)))) {
    throw $.abortCode(Std.Error.invalid_argument_($.copy(E_WRONG_SIGNATURE_SIZE), $c));
  }
...
export function new_unvalidated_public_key_from_bytes_ (
  ...
  if (!(Std.Vector.length_(bytes, $c, [AtomicTypeTag.U8])).eq(($.copy(PUBLIC_KEY_NUM_BYTES)))) {
    throw $.abortCode(Std.Error.invalid_argument_($.copy(E_WRONG_PUBKEY_SIZE), $c));
  }
...
export function new_validated_public_key_from_bytes_ (
  ...
  if (public_key_validate_internal_($.copy(bytes), $c)) {
    temp$1 = Std.Option.some_(new ValidatedPublicKey({ bytes: $.copy(bytes) }, new SimpleStructTag(ValidatedPublicKey)), $c, [new SimpleStructTag(ValidatedPublicKey)]);
  }
  else{
    temp$1 = Std.Option.none_($c, [new SimpleStructTag(ValidatedPublicKey)]);
  }
  return temp$1;
}
...
export function signature_verify_strict_t_ (
  ...
  return signature_verify_strict_internal_($.copy(signature.bytes), $.copy(public_key.bytes), Std.Bcs.to_bytes_(encoded, $c, [new SimpleStructTag(SignedMessage, [$p[0]])]), $c);
}
